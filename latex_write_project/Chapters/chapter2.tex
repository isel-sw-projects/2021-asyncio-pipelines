...
\subsection{Synchronous versus Asynchronous}
	...
	To better visualize what was explained, we have next an asynchronous enumerables example written in \text{CS}, where we can observe an asynchronous iteration over a continous data source in real time without any blockage.
	\clearpage
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[h]{1.2\textwidth}
			\centering
			\lstfromfile{sharpc}{1-30}{Asynchronous Enumerables example}{asyncenumerables}{showlines=true,morekeywords={begin,Console,async,await},numbers=left, firstnumber=1}{asyncEnum.cs}
			\caption{}
		\end{subfigure}
	\end{figure}
