...
Taking the wave initiated by the Gang of Four in \cite{gof}, where 23 patterns were compiled to deal with object-oriented problems; a group of researchers indentified characteristics that high available server must have, and how the use of asynchronous IO design patterns can help to achieve these caracheristics that are described bellow:
\begin{itemize}
...
The authors propose the \textit{Proactor Pattern}. In their opinion, conventional concurrency models fail to fully achieve the enumerated properties. In the document, before presenting the \textit{Proactor Pattern}, are identified two major existing concurrency models, namely: \textit{multithreading} and \textit{reactive event dispatching}. 
