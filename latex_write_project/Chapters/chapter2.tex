...
This section, aims to provide the background and the current state of the art on the subject related with asynchronous data operations involving asynchronous data flows.  
On the section \ref{sec:related_work}, will be made an overview on previously developted work made on this subject, then, on section \ref{sec:async_concepts}, will be made a characterization of the key concepts related to it. By last, on section \ref{sec:state_of_the_art}, are presented and explained several technologies representative of the state of the Art on how asynchronous data flows is handled in different programming realities, e.g. on Kotlin, JAVA and C\#. 
...
The authors propose in their paper the \textit{Proactor Pattern}, because, in their opinion, conventional concurrency models fail to fully achieve the enumerated properties.
...
\subsection{Synchronous versus Asynchronous}
	...
	Yet, Another example of 'intrinsic words' used in asynchronous processing is the \texttt{for await...of}, used in javascript to process asynchronous enumerables, that will be described in more detail ahead. 
	
	To better visualize the advantage of using intrinsic words in asynchronous code, on the next example, we can see a code comparation in javascript, with and without the use of intrisic words in asynchronous code, where it is possible to observe the reduce of code complexity and increment of readability 
...
First of all, in the section \ref{reactivex}, will be presented the multi-language \texttt{reactivex.io} project, and how the \texttt{Observer Pattern} is used in this apporach to implement real time asynchronous processing with and without \textit{back-pressure}.
...
\label{csenums}
	...
	In its turn, \texttt{IEnumerator<T>}, is an interface that allows to iterate over items of a Collection by offering a method \texttt{T getCurrent()}, that allows to retrieve the current item 
	...
	To better understand what was explained, we have the following diagram that will help to explain what is the class structure implemented in C\# that allow the enumeration of 
